...
class Composer extends React.Component {
  ...
  handleDrag = (event, kind, i,j ) => {
    ...
    const drop = (event) => {
      ...
      if(i !== undefined) { // do update gates
    ...
    this.props.gates.forEach((qubit, ii) => {
      ...
      qubit.forEach((gate, jj) => {
        if(i === dropI && j === dropJ) {
          ...
          <rect x={30 + 45*j} y={40+50*i-16} width={SIZE} height={SIZE} fill="white"stroke="black" strokeDasharray="4 1" strokeWidth="1"/>
          </g>);
          j++;
        }
        gates.push(<g key={key++}>
          <rect onMouseDown={(event) => this.handleDrag(event, gate, ii, jj)} x={30 + 45*j} y={40+50*i-16} width={SIZE} height={SIZE} fill="white"stroke="black" strokeWidth="2"/>
          <text onMouseDown={(event) => this.handleDrag(event, gate, ii, jj)} className="select-none" x={30+ 45*j +12} y={40+50*i+4} fontSize="12px" fontWeight="800">{gate}</text>
        </g>);
        j++;
      });
      if(i === dropI && j === dropJ) {
